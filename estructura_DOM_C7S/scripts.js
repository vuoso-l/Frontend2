...
listadoFelinos.forEach(lista => {    
    const title = document.createTextNode(lista.title);
    const description = document.createTextNode(lista.description);
    const createdAt = document.createTextNode("Created at: " + lista.createdAt);
    const item = document.createElement("div");
    item.setAttribute("class", "item");
    const img = document.createElement("img");
    img.setAttribute('src', lista.imgUrl);
    const h2 = document.createElement("h2");
    const p = document.createElement("p");
    const pCreatedAt = document.createElement("p");
    h2.appendChild(title);
    p.appendChild(description);
    pCreatedAt.appendChild(createdAt);
    item.appendChild(img);
    item.appendChild(h2);
    item.appendChild(p);
    item.appendChild(pCreatedAt);
